...
/* Note: we will migrate the cuda implementation to PyTorch in the next version */
//#define MST_PRIM
//#define MST_KRUSKAL
#define MST_BORUVKA
...
at::Tensor mst_forward(
    ...
    // Loop for batch
    auto dispatch_lambda = [&](auto scalar_type) {
    ...
    switch (edge_weight_cpu.scalar_type()) {
        case at::ScalarType::Double:
            dispatch_lambda((double)0);
            break;
        case at::ScalarType::Float:
            dispatch_lambda((float)0);
            break;
        case at::ScalarType::Half:
            dispatch_lambda((at::Half)0);
            break;
        default:
            AT_ERROR("Unsupported data type for MST forward pass");
    }
