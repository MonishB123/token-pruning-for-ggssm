...
def get_optimizer(optim_name, model, args):
    ...
    if optim_name == "adam":
        optimizer = torch.optim.Adam(
            model_weights, lr=args.learning_rate, weight_decay=args.reg_weight
        )
        ...
        optimizer = adabound.AdaBound(
            model_weights, lr=args.learning_rate, final_lr=0.1
        )
    elif optim_name == "prodigy":
        from prodigyopt import Prodigy

        # you can choose weight decay value based on your problem, 0 by default
        optimizer = Prodigy(model_weights, lr=1.0, weight_decay=args.reg_weight)
...
def get_lr_scheduler(optimizer, total_steps, args):
    if args.lr_schedule == "warmup_cosine":
        ...
        scheduler = LinearWarmupCosineAnnealingLR(
            optimizer, warmup_epochs=args.warmup_steps, max_epochs=total_steps
        )
