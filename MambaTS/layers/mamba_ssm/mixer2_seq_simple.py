...
class MixerTSModel(nn.Module):
    def __init__(
        ...
        if self.VPT_mode > 0:
            assert (
                n_vars is not None
            ), f"When VPT_mode ({self.VPT_mode}) > 0, n_vars should be passed in."
        ...
        self.adjacency_matrix = nn.Parameter(
            torch.zeros(n_vars, n_vars), requires_grad=False
        )
        self.count_matrix = nn.Parameter(
            torch.zeros(n_vars, n_vars), requires_grad=False
        )
    ...
    def forward(self, x, inference_params=None, context_len=None, **mixer_kwargs):
        # TODO: flexible for n_vars
        n_vars_cur = self.n_vars
        if self.VPT_mode in [1]:
            if self.training:
                x = rearrange(x, "b (c l) d -> b c (l d)", c=n_vars_cur)
                x, ids_shuffle, ids_restore = random_shuffle(
                    x, mask_ratio=0, return_ids_shuffle=True
                )
                # Record the starting point
                ids_shuffle_ = torch.cat([ids_shuffle[:, [0]], ids_shuffle], dim=-1)
                self.transition_tuple = torch.stack(
                    [ids_shuffle_[:, :-1], ids_shuffle_[:, 1:]], dim=-1
                )
                ...
                if self.ids_shuffle is None:
                    if self.ATSP_solver == "GD":
                        ...
                        distance_matrix[:, [start_index, 0]] = distance_matrix[
                            :, [0, start_index]
                        ]  # swap columns
                        distance_matrix[[start_index, 0], :] = distance_matrix[
                            [0, start_index], :
                        ...
                        if self.ATSP_solver == "SA":
                            shortest_path, distance = solve_tsp_simulated_annealing(
                                distance_matrix, x0=list(range(n_vars_cur))
                            )
                        elif self.ATSP_solver == "LS":
                            shortest_path, distance = solve_tsp_local_search(
                                distance_matrix, x0=list(range(n_vars_cur))
                            )
                        elif self.ATSP_solver == "LK":
                            shortest_path, distance = solve_tsp_lin_kernighan(
                                distance_matrix, x0=list(range(n_vars_cur))
                            )
                        else:
                            raise NotImplementedError(
                                f"Don't support the ATSP sovler {self.ATSP_solver}"
                            )
        ...
        if self.VPT_mode in [1]:
            hidden_states = rearrange(
                hidden_states, "b (c l) d -> b c (l d)", c=n_vars_cur
            )
            hidden_states = unshuffle(hidden_states, ids_restore)
            hidden_states = rearrange(
                hidden_states, "b c (l d) -> b (c l) d", d=self.d_model
            )
    ...
    def batch_update_state(self, cost_tensor):
        ...
        self.adjacency_matrix.view(-1).scatter_add_(
            0, indices.view(-1), cost_tensor.repeat(indices.shape[-1])
        )
        self.count_matrix.view(-1).scatter_add_(
            0, indices.view(-1), count_tensor.repeat(indices.shape[-1])
        )
