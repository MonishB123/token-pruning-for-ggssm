...
class Mamba2TS(nn.Module):
    def __init__(
        ...
        self.activation = activation
        # self.chunk_size = chunk_size
        self.chunk_size = n_vars
        self.use_mem_eff_path = use_mem_eff_path
        self.layer_idx = layer_idx

        self.use_casual_conv = use_casual_conv
        self.n_vars = n_vars
        self.dropout = dropout  # don't save it

        # Order: [z, x, B, C, dt]
        d_in_proj = 2 * self.d_inner + 2 * self.ngroups * self.d_state + self.nheads
        if self.dropout > 0:
            self.in_proj = nn.Sequential(
                nn.Linear(self.d_model, d_in_proj, bias=bias, **factory_kwargs),
        ...
        if self.use_casual_conv:
            ...
            print(f"use_casual_conv is {self.use_casual_conv}")
        # self.conv1d.weight._no_weight_decay = True
