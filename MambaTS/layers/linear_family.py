...
class GaussianLinear(nn.Module):
    def __init__(self, in_features, out_features, mean_init=0.0, std_init=1.0):
        ...
        self.mean = nn.Parameter(
            torch.Tensor(out_features, in_features).fill_(mean_init)
        )
        self.log_stddev = nn.Parameter(
            torch.Tensor(out_features, in_features).fill_(math.log(std_init))
        )
...
class FastGaussianLinear(nn.Module):
    def __init__(self, in_features, out_features, mean_init=0.0, std_init=1.0):
        ...
        self.log_stddev_weight = nn.Parameter(
            torch.Tensor(out_features, in_features).fill_(math.log(std_init))
        )
...
class LaplacianLinear(nn.Module):
    def __init__(self, in_features, out_features, loc_init=0.0, scale_init=1.0):
        ...
        self.log_scale = nn.Parameter(
            torch.Tensor(out_features, in_features).fill_(math.log(scale_init))
        )
...
class StudentTLinear(nn.Module):
    def __init__(
        self, in_features, out_features, df_init=1.0, loc_init=0.0, scale_init=1.0
    ):
        super().__init__()
        self.in_features = in_features
        self.out_features = out_features
    ...
    def forward(self, x):
        weight = torch.distributions.StudentT(
            df=self.df, loc=self.loc, scale=self.log_scale.exp()
        ).sample()
        return F.linear(x, weight)
...
class MixtureGaussianLinear(nn.Module):
    def __init__(
        self, in_features, out_features, num_mixtures=5, mean_init=0.0, std_init=1.0
    ):
        super().__init__()
        self.in_features = in_features
        self.out_features = out_features
        ...
        self.means = nn.Parameter(
            torch.Tensor(num_mixtures, out_features, in_features).fill_(mean_init)
        )
        self.log_stddevs = nn.Parameter(
            torch.Tensor(num_mixtures, out_features, in_features).fill_(
                math.log(std_init)
            )
        )
        self.mixture_weights = nn.Parameter(
            torch.Tensor(num_mixtures).fill_(1 / num_mixtures)
        )

    def forward(self, x):
        weights = torch.zeros_like(self.means)
        for i in range(self.num_mixtures):
            weights[i] = torch.distributions.Normal(
                self.means[i], self.log_stddevs[i].exp()
            ).sample()
...
class GammaLinear(nn.Module):
    def __init__(
        self, in_features, out_features, concentration_init=1.0, rate_init=1.0
    ):
        super().__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.concentration = nn.Parameter(
            torch.Tensor(out_features, in_features).fill_(concentration_init)
        )
        self.rate = nn.Parameter(
            torch.Tensor(out_features, in_features).fill_(rate_init)
        )
...
class BetaLinear(nn.Module):
    def __init__(self, in_features, out_features, alpha_init=1.0, beta_init=1.0):
        ...
        self.alpha = nn.Parameter(
            torch.Tensor(out_features, in_features).fill_(alpha_init)
        )
        self.beta = nn.Parameter(
            torch.Tensor(out_features, in_features).fill_(beta_init)
        )
...
class BayesianLinearLayer(nn.Module):
    def __init__(self, input_dim, output_dim, prior_mean=0, prior_std=1):
        ...
        self.weight_mu = nn.Parameter(
            torch.Tensor(output_dim, input_dim).uniform_(-0.2, 0.2)
        )
        self.weight_rho = nn.Parameter(
            torch.Tensor(output_dim, input_dim).uniform_(-5, -4)
        )
